...
## Design-system tokens (accent hues, card tiers)
- `globals.css` defines 8 named accent hues (`--accent-rose/terracotta/amber/olive/emerald/teal/sky/indigo` + `-soft` variants) with dark-mode overrides, plus `.card-flat` (soft grouping bg, no border), `.card-raised` (card bg + shadow, no border), and `.section-label` (small-caps primary). New colour-coded UI should use these tokens, not inline `bg-[oklch(...)]`.
- When verifying token changes, always toggle dark mode (theme button in topbar, left of "Foundations"; it cycles system → light → dark) — missing dark overrides show as illegible light chips on dark bg.
- Mastery dots on vocab cards are grey at mastery 0; to see coloured (gold) dots on camera, answer ~3 cards of a lesson MC deck correctly, then open `/vocabulary` — the practiced words show gold dots. This doubles as a regression check that practice still updates progress/daily goal.
- Gender chips in `/vocabulary`: masc = sky/blue, fem = rose/pink; use the "Any gender" filter to isolate each.

## Mobile layout testing (immersive mode, tab bar, auto-hiding topbar)
- Simulate a phone by resizing the Chrome window to ~400-420px wide (Tailwind `sm` is 640px, `md` 768px); use `wmctrl` to re-maximize for desktop regression afterwards. This doesn't exercise touch scrolling, iOS URL-bar collapse (`100dvh`), or safe-area insets — note that limitation in the report.
- Expected phone behavior (from PR #64): a fixed 5-tab bottom bar (`src/components/tab-bar.tsx`, `md:hidden`), a single horizontally-scrollable nav row + icon-only streak/goal chips in the topbar, and the topbar auto-hiding on scroll down / reappearing on scroll up (`useHideOnScroll` in topbar.tsx).
- During a running deck, `body.session-active` (set by exercise-runner.tsx) hides topbar/tab bar/footer via CSS on phones; the whole question should fit the viewport, and the feedback bar is a `position: fixed` bottom sheet on phones (inline in the card on ≥sm — verify both).
- Fixed-positioning gotcha: any ancestor retaining a `transform` (e.g. an entry animation with `fill-mode: both`) becomes the containing block for `position: fixed` and breaks the bottom sheet. If the sheet floats mid-card, check animation fill modes (`backwards` is safe).
- When clicking browser-chrome-level targets at narrow widths, the Chrome autocomplete dropdown may hijack typed URLs (e.g. "/" autocompletes to "/practice"); prefer clicking in-app nav links instead of retyping URLs.
